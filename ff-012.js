// ==========================================================
//  FF-012 — Controle de Defeitos Detectados na Inspeção
//  Tabela única (log): 1 linha por inspeção, contagem por tipo de defeito.
...
const CODIGO_FF = "ff012";
const NOME_FF = "Controle de Defeitos Detectados na Inspeção";
...
function desenharHistorico() {
  ...
  registrosAtuais.forEach(function (r) {
    ...
    html += "<td>" + tx(c.qtdeProgramada) + "</td>";
    html += "<td>" + tx(r.cliente) + "</td>";
    html += "<td>" + tx(c.data) + "</td>";
    html += "<td>" + tx(c.hora) + "</td>";
    html += "<td>" + tx(c.operador) + "</td>";
    html += "<td>" + tx(c.contaminacao) + "</td>";
    html += "<td>" + tx(c.marcasArranhoes) + "</td>";
    html += "<td>" + tx(c.trincas) + "</td>";
    html += "<td>" + tx(c.deformacoes) + "</td>";
    html += "<td>" + tx(c.rebarbaCantos) + "</td>";
    html += "<td>" + tx(c.mistura) + "</td>";
    html += "<td>" + tx(c.pecasSujas) + "</td>";
    html += "</tr>";
  });
...
function abrirInclusao() {
  ...
  h += "<td><input type='number' id='n-qprog' class='in-mini' inputmode='numeric' min='0'></td>";
  h += "<td><span id='n-cliente' class='cliente-auto'>—</span></td>";
  h += "<td><input type='date' id='n-data' class='in-mini'></td>";
  h += "<td><input type='time' id='n-hora' class='in-mini'></td>";
  h += "<td><input type='text' id='n-operador' class='in-mini' placeholder='Nome e sobrenome'></td>";
  h += "<td><input type='number' id='n-contam' class='in-mini' inputmode='numeric' min='0'></td>";
  h += "<td><input type='number' id='n-marcas' class='in-mini' inputmode='numeric' min='0'></td>";
  h += "<td><input type='number' id='n-trincas' class='in-mini' inputmode='numeric' min='0'></td>";
  h += "<td><input type='number' id='n-deform' class='in-mini' inputmode='numeric' min='0'></td>";
  h += "<td><input type='number' id='n-rebarba' class='in-mini' inputmode='numeric' min='0'></td>";
  h += "<td><input type='number' id='n-mistura' class='in-mini' inputmode='numeric' min='0'></td>";
  h += "<td><input type='number' id='n-sujas' class='in-mini' inputmode='numeric' min='0'></td>";
...
async function salvarLinhaNova() {
  ...
  const operador = document.getElementById("n-operador").value.trim();

  if (!op) { alert("Selecione a Ordem de Produção."); return; }
  if (!data) { alert("Informe a data da inspeção."); return; }
  if (!operador) { alert("Informe o nome do operador."); return; }
  if (operador.split(/\s+/).filter(function (p) { return p.length > 0; }).length < 2) {
    alert("Digite nome e sobrenome do operador."); return;
  }
  ...
  const registro = {
    ...
    campos: {
      numeroOp: op.numero || "",
      cliente: op.cliente || "",
      qtdeProgramada: v("n-qprog"),
      data: data,
      hora: v("n-hora"),
      operador: operador,
      contaminacao: v("n-contam"),
      marcasArranhoes: v("n-marcas"),
      trincas: v("n-trincas"),
      deformacoes: v("n-deform"),
      rebarbaCantos: v("n-rebarba"),
      mistura: v("n-mistura"),
      pecasSujas: v("n-sujas")
    }
  };
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        lista.push({
          ...
          responsavel: operador, criadoEm: registro.criadoEm, campos: registro.campos
        });
